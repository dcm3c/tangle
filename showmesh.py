...
def next_data_line(f):
    """Read the next non-comment, non-blank line from a Triangle-format file."""
    while True:
        line = f.readline()
        if not line:
            return None
...
def main():
    ...
    has_regions = any(a != 0 for a in attrs)
    show_regions = args.regions or has_regions  # auto-enable if regions present

    if show_regions and has_regions:
        # Color by region using distinct colors
        triang = mtri.Triangulation(x, y, tri_array)
        region_ids = sorted(set(attrs))
        n_regions = len(region_ids)
        # Use tab20 for up to 20 regions, tab20b+tab20c beyond that
        if n_regions <= 20:
            cmap = plt.cm.tab20
        else:
            cmap = plt.cm.gist_ncar
        # Map region attributes to sequential color indices
        region_to_idx = {r: i for i, r in enumerate(region_ids)}
        colors = np.array([cmap(region_to_idx[a] / max(n_regions, 1)) for a in attrs])
        from matplotlib.collections import PolyCollection
        triangles_xy = np.array([
            [[x[tri_array[i,j]], y[tri_array[i,j]]] for j in range(3)]
            for i in range(len(tri_array))
        ])
        pc = PolyCollection(triangles_xy, facecolors=colors,
