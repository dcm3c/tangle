...
| `-C`   | PSLG cleanup at a coarser tolerance: merge near-duplicate nodes, split intersecting segments, remove duplicates. Optional value (e.g. `-C0.001`); default is bounding-box diagonal × 1e-6. (A near-exact hygiene pass — × 1e-12 — runs on every PSLG regardless, catching exact duplicates and crossing segments.) |
...
Algorithms implemented by tangle include the following.
...
their midpoint and the process repeats. Constrained edges are then
respected as barriers during all subsequent operations.
...
Bad triangles are processed shortest-edge first. New vertices are inserted into the existing triangulation using [Lawson's incremental insertion algorithm](https://en.wikipedia.org/wiki/Delaunay_triangulation#Flip_algorithms) ([Lawson 1977](https://ntrs.nasa.gov/api/citations/19770025881/downloads/19770025881.pdf)), which splits the containing triangle and restores the Delaunay property through local edge flips.
...
### 10. Geometric Robustness — Extended Precision Arithmetic

The geometric predicates `orient2d` (point-line orientation) and `inCircle` (point-in-circumcircle) use a floating-point filter with an extended precision arithmetic fallback. Each predicate first evaluates a fast double-precision (~53-bit) computation, then checks the result against [a forward error bound derived from the operand magnitudes](https://www.femm.info/wiki/PredicateBounds) ([Higham 2002](https://www.google.com/books/edition/Accuracy_and_Stability_of_Numerical_Algo/epilvM5MMxwC)). When the result is too close to zero relative to the bound to determine the sign reliably, the computation falls back to extended-precision arithmetic. The extended precision is provided by a custom floating-point type, **`float256`**, implemented as one IEEE double (carrying the sign, exponent, and top 52 mantissa bits) plus three unsigned 64-bit integers (`uint64_t`) that extend the mantissa to ~245 bits.

This two-tier approach (double → float256) avoids the robustness failures that plague double-precision floating-point implementations while keeping the common case fast. Over 99% of predicate calls are resolved at double precision; only the rare near-degenerate cases fall through to the extended precision tier.

## File Formats

Tangle uses the same file formats as Triangle for `.node`, `.poly`, `.ele`, `.edge`, and `.neigh` files. See the [Triangle documentation](https://www.cs.cmu.edu/~quake/triangle.html) for the base format, and [`polyformat.txt`](https://www.femm.info/wiki/PolyFormat) for tangle's extensions (LFS columns, arc segments, PBC definitions, and `.pbc` output).
